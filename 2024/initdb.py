...
# TODO : 538 is dead. Let's hope Nate Silver's "Silver Bulletin" is backwards compatible :)
#
...
def load538Forecast(teams_lookup):
    path = year + '/data/fivethirtyeight_ncaa_forecasts.csv'
    ...
    for line in raw_data[1 : 69]:
        ...
        # TODO : Nate dropped the rating field.
        #teams_lookup[name].rating = float(vals[keys['team_rating']])
        teams_lookup[name].play_in = vals[keys['playin_flag']] == "1"
    
    load538Elo(teams_lookup)

# We have a separate data source for the ELO of each team
def load538Elo(teams_lookup):
    path = year + '/data/elo.csv'
    raw_data = open(path).read().split('\n')
    ...
    for line in raw_data[1:]:
        ...
        name = vals[0]
        rating = float(vals[-1])
        teams_lookup[name].rating = rating
...
def loadGames(teams):
    ...
    for team in teams:
        # Convert seed from string to int. Strip 'a' or 'b' off play in teams, who we assume have seeds >= 10. (That is, their seed string is 3 characters long.)
        region_seed = int(team.seed[:2])

        gid = offsets[team.region] + region_seed_map[region_seed]
        is_team1 = region_seed <= 8

        # First Four teams are special. They play for the team2 slot of the game in the Round of 64.
        if team.play_in:
            gid = 2 * gid + 1
            is_team1 = team.seed[2] == 'a'
...
def writeCheatSheet(brackets, streak_gids):
    path = year + "/data/cheat_sheet.txt"
...
def writeSortableCheatSheet(brackets, streak_gids):
    path = year + "/data/sortable_cheat_sheet.csv"
    with open(path, "w+") as file:
        file.write("ID,Chalk Score,538 Score,HEAT Score,Winner,Runner Up,Purdue Depth,UVA Depth\n")
        for bracket in brackets:
            str_chalk = str(round(bracket.calcChalkScore(chalk), 2))
            str_538 = str(round(bracket.calc538Score(), 2))
            str_heat = str(round(bracket.calcHeatScore(streak_gids), 4))
            ...
            # Write the information to the file
            file.write("%s,%s,%s,%s,%s,%s,%s,%s\n" % (str(bracket.bid), str_chalk, str_538, str_heat, ordered[0], ordered[1], purdue, uva))
...
for i in range(kTotalBrackets):
    ...
    path = year + "/data/brackets/%s.txt" % bid
